...
def update_pin1_neighbours():

	for y in range(ip.tissue_rows):
		for x in range(ip.tissue_columns):

			# Top cell, Bottom face
			if y > 0:
				ip.pin1_neighbours[0,y,x] = ip.pin1[2,y-1,x]
			else:
				ip.pin1_neighbours[0,y,x] = ip.pin1[0,y,x]

			# Right cell, Left face
			if x < ip.tissue_columns-1:
				ip.pin1_neighbours[1,y,x] = ip.pin1[3,y,x+1]
			else:
				ip.pin1_neighbours[1,y,x] = ip.pin1[1,y,x]

			# Bottom cell, Top face
			if y < ip.tissue_rows-1:
				ip.pin1_neighbours[2,y,x] = ip.pin1[0,y+1,x]
			else:
				ip.pin1_neighbours[2,y,x] = ip.pin1[2,y,x]

			# Left cell, Right face
			if x > 0:
				ip.pin1_neighbours[3,y,x] = ip.pin1[1,y,x-1]
			else:
				ip.pin1_neighbours[3,y,x] = ip.pin1[3,y,x]
